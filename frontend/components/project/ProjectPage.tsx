...
function mapApiTask(task: ApiTask): TaskItem {
  return { ...task, assigneeId: task.assignee?.id, assignee: task.assignee?.displayName || "ยังไม่มอบหมาย", dueDate: fromISODate(task.dueDate), meetingId: task.meetingId || undefined, feedbackId: task.feedbackId || undefined };
}
...
export default function ProjectPage() {
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter((t) => {
      ...
      const matchesAssignee = taskAssigneeFilterId === null || t.assigneeId === taskAssigneeFilterId;
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  {taskAssigneeFilterId && (
                    <button
                      ...
                      onClick={() => setTaskAssigneeFilterId(null)}
                      style={{ display: "inline-flex", alignItems: "center", gap: "5px", padding: "4px 10px", fontSize: "11px", height: "34px" }}
                      aria-label="ยกเลิกตัวกรองผู้รับผิดชอบ"
                    >
                      ผู้รับผิดชอบ: {members.find((member) => member.id === taskAssigneeFilterId)?.name || ""}
                      <X className="w-3 h-3" />
                    </button>
                  )}
                </div>
